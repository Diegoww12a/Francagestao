  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-white">Missões Agendadas</h2>
          <p className="text-gray-400 mt-1">Planeje e acompanhe missões futuras</p>
        </div>
        <button
      ...
      {isAdding && (
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
          ...
          <div className="space-y-4">
            ...
            <input
              type="datetime-local"
              value={newMission.scheduled_date}
              onChange={(e) => setNewMission({ ...newMission, scheduled_date: e.target.value })}
              className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500"
            />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div key={mission.id} className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                {mission.status !== 'urgent' && mission.status !== 'completed' && (
                  <button
                    onClick={() => updateMissionStatus(mission.id, 'urgent')}
                ...
                {mission.status === 'completed' && (
                  <button
                    onClick={() => updateMissionStatus(mission.id, 'pending')}
        ...
        {missions.length === 0 && (
          <div className="col-span-2 text-center py-12">
            <p className="text-gray-500">Nenhuma missão agendada</p>
          </div>
        )}
      </div>
    </div>
