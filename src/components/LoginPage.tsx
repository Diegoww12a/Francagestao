...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  try {
    const res = await fetch('https://francagestao.onrender.com/login', {
      method: 'POST',
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-gray-900/80 backdrop-blur border border-gray-800 rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 disabled:from-gray-700 disabled:to-gray-800 text-white font-semibold py-3 rounded-lg transition-all duration-200 transform hover:scale-[1.02] active:scale-95"
            >
              {isLoading ? (
          ...
          <div className="mt-8 pt-6 border-t border-gray-800">
            <p className="text-xs text-gray-500 text-center">
              Sistema seguro de gerenciamento
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-500 text-sm">
            Painel Administrativo da  França
          </p>
        </div>
      </div>
    </div>
